          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                <div className="bg-[#1e1b4b]/30 p-5 rounded-2xl border border-indigo-500/20">
                  <p className="text-indigo-400 text-xs font-semibold tracking-wide uppercase mb-3">Deployed By</p>
                  <div className="flex items-center gap-4">
                    {selectedCommit.author?.avatar_url ? (
                      <img src={selectedCommit.author.avatar_url} alt="Committer" className="w-12 h-12 rounded-full border border-indigo-500/30 shadow-md" />
                    ) : (
                      <div className="w-12 h-12 rounded-full bg-indigo-500/20 flex items-center justify-center border border-indigo-500/30 text-indigo-300 font-bold">
                        {selectedCommit.commit.author.name.charAt(0).toUpperCase()}
                      </div>
                    )}
                    <div>
                      <p className="text-indigo-50 font-bold text-lg">{selectedCommit.commit.author.name}</p>
                      <p className="text-slate-400 text-sm font-mono">{selectedCommit.commit.author.email}</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-col gap-4 flex-1">
                  <div className="flex flex-col gap-3">
                    <textarea 
                      ...
                      className="w-full bg-[#0f172a] border border-indigo-500/40 rounded-xl p-4 text-sm text-white focus:outline-none focus:border-indigo-400 shadow-inner resize-none h-24 leading-relaxed" 
                      placeholder="Input query paradigm..." 
                    />
                    <button 
                      onClick={handleAiSummarize} 
                      disabled={isAiLoading || !aiPrompt.trim()} 
                      className="self-end bg-indigo-600 hover:bg-indigo-500 px-8 py-3 rounded-xl text-sm font-bold transition-colors disabled:opacity-50 flex items-center gap-2 shadow-lg hover:shadow-indigo-500/50"
                    >
